...
typedef struct stBlock StBlock;
/***************************************************************************/
...
typedef struct stHuffmanTable StHuffmanTable;
...
typedef struct stComponent StComponent;
...
typedef struct stJpegData StJpegData;
...
//		jdata->m_rgb = new unsigned char[width * height];
        jdata->m_rgb =  malloc(sizeof(unsigned char)*(width * height));
    ...
    int x,y;
printf("\ny= %d  %d\n",jdata->m_height,ystride_by_mcu);
printf("x= %d  %d\n",jdata->m_width,xstride_by_mcu);
...
//	StJpegData* jdec = new StJpegData();
    StJpegData* jdec = malloc(sizeof(StJpegData));
...
//	buf = new unsigned char[lengthOfFile + 4];
    buf =  malloc(sizeof(unsigned char)*(lengthOfFile + 4));
	if (buf == NULL)
	{
