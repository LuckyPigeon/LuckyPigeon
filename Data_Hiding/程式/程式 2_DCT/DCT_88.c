...
int main()
{
  ...
  double dct_before[8][8];
  double dct_after[8][8];
  double idct_before[8][8];
  double idct_after[8][8];
  ...
  for (m=0;m<(SIZE/8);m++){
    for (n=0;n<(SIZE/8);n++){
      ...
      for (i=0;i<8;i++){
        for (j=0;j<8;j++){
          dct_before[i][j] = (double)lena[i+m*8][j+n*8];
        }
      }
...
//      lena_dc[m][n]= (char)dct_after[0][0]; // dc sub-image
      lena_dc[m][n]= dct_after[0][1]; // AC sub-image
//    lena_dc[m][n]= dct_after[1][0]; // AC sub-image

      for (i=0;i<8;i++){
        for (j=0;j<8;j++){
           *(dct_total+SIZE*(i+m*8)+j+n*8) = dct_after[i][j];         
        }
      }
    }
  }
...
//  
  ...
  for (m=0;m<(SIZE/8);m++){
    for (n=0;n<(SIZE/8);n++){
      ...
      for (i=0;i<8;i++)
        for (j=0;j<8;j++)
          idct_before[i][j] = *(dct_total+SIZE*(i+m*8)+j+n*8);

      idct_func(idct_before,idct_after);

      for (i=0;i<8;i++)
        for (j=0;j<8;j++)
          lena[i+m*8][j+n*8] = (char)idct_after[i][j];
    }
  }
