...
void init_CPU_cycle_time(void){
	...
	/* HOMEWORK: finish the CPU cycle time calculation for multi-cycle datapath and pipeline datapath */
	switch(DATAPATH_TYPE){
		case SINGLE://5個全部
			CPU_CYCLE_TIME = MEMORY_ACC_TIME * 2 + REGISTER_ACC_TIME * 2 + ALU_TIME;  
			break; 
		case MULTI: //以5個裡最長的為準
			CPU_CYCLE_TIME = MEMORY_ACC_TIME > REGISTER_ACC_TIME ? MEMORY_ACC_TIME : REGISTER_ACC_TIME;
			CPU_CYCLE_TIME = CPU_CYCLE_TIME > ALU_TIME ? CPU_CYCLE_TIME : ALU_TIME;
